      ...
      function(error, mongo) {
        ...
        db.collection(collectionSource)
          ...
          .toArray(function(err, result) {
            ...
            if (action === ACTION_BACKUP_SOURCE) backupDocuments(collectionTarget, result)
            else if (action === ACTION_APPEND) insertDocuments(collectionTarget, result)
            else if (action === ACTION_CLONE) insertDocuments(collectionTarget, result)
          ...
          .then(count => {
            count >= 0 &&
              db.collection(collectionTarget).drop(function(err, result) {
                if (err) throw err
                if (result) console.log('Target', collectionTarget, '- collection deleted')
                mongo.close()
              })
          })
      },
